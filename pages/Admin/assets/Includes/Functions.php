...
// activity responses
    ...
    function viewAnswersProgress($connection): string
    {

        $data = "";
        $nofStudents = 0;
        //get total students
        $query = "SELECT COUNT(student_id) AS STDCOUNT FROM `student`";
        $result = mysqli_query($connection, $query);
        ...
        $query = "SELECT deadline,COUNT(answer_id) AS CUNT,activity_name,activity_id FROM `activityscriptsviews` ORDER BY activity_id";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                $deadlineCl=$deadline > $current ? 'bg-success ':'bg-danger';
                $deadlinetitle=$deadline > $current ? 'Assignment not overdue ':'Assignment Overdue';
                $counttitle=$count. " responses out of ".$nofStudents." students";
                ...
                $data = $data . "  <tr >
                                    <td>{$activity_name}</td>
                                     ...
                                     <td type='button' class=' d-none d-lg-table-cell text-light {$deadlineCl}' data-toggle='tooltip' data-placement='top' title='{$deadlinetitle}'>
                                        {$deadline}
                                     </td>
                                
                                    <td type='button' class='font-weight-bold d-none d-lg-table-cell text-dark ' data-toggle='tooltip' data-placement='top' title='{$counttitle}'>
                                       <h3 class=' bg-warning'>{$count}/$nofStudents</h3>
                                     </td>
                                    ...
                                    <td>
                                        <a
                                                ...
                                                href='viewRes.php?res_type=ac&res_id={$activity_id}'
